...
def burn_subtitles_with_background_and_timecode(video_file, srt_file=None, font_size=None, opacity=0.7, smpte_only=False):
    # Ensure the provided video file exists
    ...
    # Construct the filter for SMPTE timecode
    ...
    if not smpte_only:
        # Ensure the provided SRT file exists
        ...
        filters.append(f"drawbox=x=0:y=ih-70:w=iw:h=60:color=black@{opacity}:t=fill,{subtitles_filter}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or (len(sys.argv) < 3 and '--smpte-only' not in sys.argv):
        print("Usage: python burn_subtitles_with_background_and_timecode.py <video_file> [<srt_file> [font_size] [opacity]] [--smpte-only]")
    else:
        ...
        burn_subtitles_with_background_and_timecode(video_file, srt_file, font_size, opacity, smpte_only)
